"""
optimize_model_real_data.py  (single-view, Real_SR_Data/LND_TEST)
Differentiable-rendering 6-DoF pose estimator (PyTorch3D silhouette loss).
Adapted from optimize_modelv2.py for the Real_SR_Data/LND_TEST dataset.

Three-stage optimisation (same structure as optimize_modelv2.py):
...
Loss design:
  faces_per_pixel=1 keeps the soft-rendered halo small (~1.3× binary mask
  area at sigma=0.025).  The reference for MSE is the Gaussian-blurred binary
  mask, whose spatial profile matches the soft renderer's sigmoid fall-off —
  so at GT pose the rendered alpha ≈ blurred reference → MSE ≈ 0.  This
  mirrors what optimize_modelv2.py does for simulation data (there the GT
  silhouette rendered at each sigma is the reference; here we substitute the
  blurred binary mask, which is the closest equivalent for real images).
  IoU (added in stages 2b/2c) is robust to residual mesh-vs-object mismatch.
"""
...
OUTPUT_DIR = "../Diff_Render_CSV_Visuals/Pose_Estimate_Real_Data"
...
NUM_RESTARTS      = 3     # independent restarts per frame; pick best by final loss
...
_viz_renderer = make_silhouette_renderer(cam_K[0], cam_K[4], cam_K[2], cam_K[5],
                                         ...
                                         faces_per_pixel=100)
_mesh_viz = build_mesh()  # shared across all overlay draws
...
for frame_id in valid_frame_ids:
    ...
    # ── N restarts; keep the result with lowest final Stage 2c loss ──────────
    if USE_GT_INIT_ONLY:
        init_R, init_t = perturb_pose(R_gt, t_gt, INIT_ROT_NOISE_DEG, INIT_TRANS_NOISE_MM) \
                         if USE_GT_INITIALIZATION else (np.eye(3, dtype=np.float64), mean_t_fallback.copy())
        ...
        best_R, best_T, best_loss = None, None, float('inf')
        init_rot_err = init_trans_err = 0.0
        ...
        for _restart in range(NUM_RESTARTS):
            if USE_GT_INITIALIZATION:
                ...
                _init_R = np.eye(3, dtype=np.float64)
                _init_t = mean_t_fallback.copy()

            if _restart == 0:
                init_rot_err   = rotation_error_deg(_init_R, R_gt)
                init_trans_err = translation_error_mm(_init_t, t_gt)
                print(f"  Init[0]: rot={init_rot_err:.2f}°  trans={init_trans_err:.2f} mm")

            _init_R_pt3d, _init_t_pt3d = bop_to_pt3d(_init_R, _init_t)
            model = PoseModel(
                mesh, sil_renderer_s1, mask_resized,
                _init_R_pt3d, _init_t_pt3d,
                fx_r, fy_r, cx_r, cy_r, RENDER_SIZE, RENDER_SIZE,
            ).to(device)
            ...
            loop_s1 = tqdm(range(NUM_ITERS_TRANS), desc=f"  s1  f{frame_id:04d} r{_restart}")
            for _ in loop_s1:
                opt_s1.zero_grad(); loss, _, _ = model(); loss.backward(); opt_s1.step()
                loop_s1.set_description(f"  s1  f{frame_id:04d} r{_restart} ({loss.item():.3f})")
            ...
            loop_s1b = tqdm(range(NUM_ITERS_TRANS_S1B), desc=f"  s1b f{frame_id:04d} r{_restart}")
            for _ in loop_s1b:
                opt_s1b.zero_grad(); loss, _, _ = model(); loss.backward(); opt_s1b.step()
                loop_s1b.set_description(f"  s1b f{frame_id:04d} r{_restart} ({loss.item():.2f})")
            ...
            loop_s2a = tqdm(range(NUM_ITERS_COARSE), desc=f"  s2a f{frame_id:04d} r{_restart}")
            for _ in loop_s2a:
                opt_s2a.zero_grad(); loss, _, _ = model(); loss.backward(); opt_s2a.step()
                loop_s2a.set_description(f"  s2a f{frame_id:04d} r{_restart} ({loss.item():.2f})")
            ...
            loop_s2b = tqdm(range(NUM_ITERS_MID), desc=f"  s2b f{frame_id:04d} r{_restart}")
            for _ in loop_s2b:
                opt_s2b.zero_grad(); loss, _, _ = model(); loss.backward(); opt_s2b.step()
                loop_s2b.set_description(f"  s2b f{frame_id:04d} r{_restart} ({loss.item():.2f})")
            ...
            loop_s2c = tqdm(range(NUM_ITERS_JOINT), desc=f"  s2c f{frame_id:04d} r{_restart}")
            for _ in loop_s2c:
                opt_s2c.zero_grad(); loss, _, _ = model(); loss.backward(); opt_s2c.step()
                loop_s2c.set_description(f"  s2c f{frame_id:04d} r{_restart} ({loss.item():.2f})")

            with torch.no_grad():
                _final_loss, _, _ = model()
                _R_t = rot6d_to_matrix(model.rot6d)
                _T_t = model.transl
            _R_pt3d = np.from_dlpack(_R_t.detach().cpu())
            _T_pt3d = np.from_dlpack(_T_t.detach().cpu())
            _R_bop, _T_bop = pt3d_to_bop(_R_pt3d, _T_pt3d)

            print(f"  Restart {_restart+1}/{NUM_RESTARTS}: final_loss={_final_loss.item():.4f}"
                  f"  rot={rotation_error_deg(_R_bop, R_gt):.1f}°  trans={translation_error_mm(_T_bop, t_gt):.1f}mm")

            if _final_loss.item() < best_loss:
                best_loss = _final_loss.item()
                best_R = _R_bop.copy()
                best_T = _T_bop.copy()

        R_est_np, T_est_np = best_R, best_T
