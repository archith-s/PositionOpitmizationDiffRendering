...
class Evaluator:
    ...
    def summarize(self,save_path=None):
        ...
        # Avg Acc (0-5mm): SurgRIPE paper's (arxiv 2501.02990) Table 4 column —
        # "average accuracy metric which is calculated by averaging the ADD
        # errors for threshold values ranging from 0 mm to 5 mm", i.e. the mean
        # of the ADD pass-rate at each integer threshold 0,1,2,3,4,5mm. Distinct
        # from `add` (single pass-rate at the 10%-diameter threshold).
        add_dists_arr = np.array(add_dists, dtype=float)
        if len(add_dists_arr) > 0:
            avg_acc_0_5mm = float(np.mean([(add_dists_arr <= t).mean() for t in [0, 1, 2, 3, 4, 5]]))
            # Avg Acc (0-10mm): NOT a paper metric — arxiv 2501.02990 only ever
            # defines/uses the 0-5mm version (confirmed across Tables 4-7, both
            # instruments). This extends the same discrete-integer-threshold
            # averaging style out to 10mm purely as extra diagnostic info; it
            # is not comparable to anything in the paper's tables.
            avg_acc_0_10mm = float(np.mean([(add_dists_arr <= t).mean() for t in range(0, 11)]))
        else:
            avg_acc_0_5mm = 0.0
            avg_acc_0_10mm = 0.0
        ...
        print('Avg Acc (0-5mm): {}'.format(avg_acc_0_5mm))
        print('Avg Acc (0-10mm, not a paper metric): {}'.format(avg_acc_0_10mm))
        ...
        self.trans_error = []
        self.rot_error = []
        results = {'proj2d': proj2d, 'add': add,'add-s': adds, 'ADD-distance': add_dist, 'ADD-distance list': add_dists, 'ADDS-distance': adds_dist, 'cmd5': cmd5, 'trans_error': trans_error, 'rot_error':rot_error, 'avg_acc_0_5mm': avg_acc_0_5mm, 'avg_acc_0_10mm': avg_acc_0_10mm}
        if self.is_save_pose:
            results['pred_list'] = self.pred_list
            results['gt_list'] = self.gt_list
        if save_path is not None:
            np.save(save_path,results)
        return {'proj2d': proj2d, 'add': add, 'add-s': adds, 'ADD-distance': add_dist, 'ADDS-distance': adds_dist, 'cmd5': cmd5, 'trans_error': trans_error, 'rot_error':rot_error, 'avg_acc_0_5mm': avg_acc_0_5mm, 'avg_acc_0_10mm': avg_acc_0_10mm}